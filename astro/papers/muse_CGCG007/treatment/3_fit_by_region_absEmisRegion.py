...
for i, obj in enumerate(objList):
    ...
    # for idx_region in [1]:
        ...
        idcs_voxels = np.array([(176,150),(177,150),(178,150),(177,149),(176,149),(176,148)])
        n_voxels = idcs_voxels.shape[0]
        ...
        # for idx_voxel in np.arange(n_voxels):
            ...
            peaks_table, matched_masks_DF = voxel.match_line_mask(mask_df, noise_region)


            if verbose:
                voxel.plot_spectrum(peaks_table=peaks_table, match_log=matched_masks_DF, spec_label=f'{obj} voxel {idx_j}-{idx_i}',
                                    log_scale=True)

            # Fit and check the regions
            obsLines = matched_masks_DF.index.values
            for j, lineLabel in enumerate(obsLines):
                print(lineLabel)
                if lineLabel == 'H1_4861A':
