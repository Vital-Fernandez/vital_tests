    ...
    for param in param_list:
        ...
        with fits.open(f'{chemFolder}/{param}.fits') as hdu_list:
            ...
            for i_mask, mask_items in enumerate(spatial_mask_dict.items()):
                ...
                save_cfg('../muse_CGCG007.ini', store_dict, section_name=f'{mask_name}_err_GridSampling')

            ax.hist(array_container, bins=15, label=data_labels, stacked=True)

            ax.legend()
            ax.update({'title': r'CGCG007−025, {} uncertainty histogram, Grid sampling'.format(param_label), 'xlabel': param_label})
            # plt.savefig(chemFolder/f'{obj}_{param}_err_histogram_GridSampling')
            plt.show()
