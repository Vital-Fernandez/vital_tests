...
# Checking the inputs from the data file
...
# Checking the inputs and assigning default values to zero
if not Label_ID:
...
if not Label_HeI_4471:
    HeI_4471 = np.zeros(input1.size)
else:
    HeI_4471 = input1['HeI_4471']
if not Label_eHeI_4471:
    eHeI_4471 = np.zeros(input1.size)
else:
    eHeI_4471 = input1['eHeI_4471']
if not Label_HeI_5876:
    HeI_5876 = np.zeros(input1.size)
else:
    HeI_5876 = input1['HeI_5876']
if not Label_eHeI_5876:
    eHeI_5876 = np.zeros(input1.size)
else:
    eHeI_5876 = input1['eHeI_5876']
if not Label_HeII_4686:
    HeII_4686 = np.zeros(input1.size)
else:
    HeII_4686 = input1['HeII_4686']
if not Label_eHeII_4686:
    eHeII_4686 = np.zeros(input1.size)
else:
    eHeII_4686 = input1['eHeII_4686']

# Output array
output = np.zeros(input1.size, dtype=[('ID', 'U12'), ('OII_3727', float), ('eOII_3727', float), ('OIII_5007', float),
                                      ('eOIII_5007', float), ('HeI_4471', float), ('eHeI_4471', float),
                                      ('HeI_5876', float), ('eHeI_5876', float), ('HeII_4686', float),
                                      ('eHeII_4686', float), ('SII_6725', float), ('eSII_6725', float),
                                      ('SIII_9069', float), ('eSIII_9069', float), ('OH', float), ('eOH', float),
                                      ('Teff', float), ('eTeff', float), ('logU', float), ('elogU', float)])

# Filling input data into the output array
output['ID'] = Names
...
# Beginning of loop through each object
...
for tab in range(0, len(input1), 1):
    ...
    # Interate through the calculationss
    for monte in range(0, n, 1):
        ...
        # Assign a value for each line flux from a normal distrubution given the error
        OII_3727_obs = 0
        if OII_3727[tab] > 0:
        ...
        # Compute ration diagnostics if possible, otherwise asign -10 value
        ...
        if logOH[tab] > 0:
            ...
            for x in range(0, bin):
                for y in range(0, 10):
                    TO_interp = grid[i0 + x, y] * np.abs(0.3 - OH + grid[i0, 0]) / 0.3 + grid[i1 + x, y] * np.abs(0.3 - grid[i1, 0] + OH) / 0.3
                    grid_T0.append(TO_interp)
        ...
        if S2S3_obs == -10 and O2O3_obs == -10 and He12a_obs == -10 and He12b_obs == -10:
            ...
            # Loop through every column of the grid
            for index in grid_T:
...
if sed == 3:
    np.savetxt(input00 + '_hcm-teff-output.dat', output,
