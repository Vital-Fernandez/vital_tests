...
# lm_osiris.plot_spectrum_components(continuumFlux=lm_osiris.errFlux, log_scale=True, axConf={'title': 'OSIRIS spectrum'})
...
# lm_muse.plot_spectrum_components(continuumFlux=lm_muse.errFlux, log_scale=True, axConf={'title': 'MUSE spectrum'})
...
# lm_xshooter.plot_spectrum_components(continuumFlux=lm_xshooter.errFlux, log_scale=True, axConf={'title': 'XSHOOTER spectrum'})
...
wave, flux, err = wave_array, flux_array[0][0], flux_array[3][0]
normFlux = 1e-18
lm_isis = sr.LineMesurer(wave, flux, input_err=err, normFlux=normFlux)
lm_isis.plot_spectrum_components(continuumFlux=lm_isis.errFlux, log_scale=False, axConf={'title': 'ISIS spectrum'})
...
# spec_address = Path('D:/Dropbox/Astrophysics/Papers/gtc_greenpeas/data/gp030321_SDSS.fits')
# wave, data_dict, header = sr.import_fits_data(spec_address, instrument='SDSS')
# flux, err, normFlux = data_dict['flux'], data_dict['ivar'], 1
# lm_osiris = sr.LineMesurer(wave, flux, input_err=err, normFlux=normFlux)
# lm_osiris.plot_spectrum_components(continuumFlux=lm_osiris.errFlux, log_scale=True, axConf={'title': 'SDSS spectrum'})
# lineWaves = np.array([5600.0, 5635.0, 5651.0, 5675.0, 5697.0, 5729.0])
# lm_osiris.fit_from_wavelengths('H1_4861A', lineWaves)
# lm_osiris.print_results(show_plot=True)
