...
for i, file_address in enumerate(addressList):
    ...
    pdfTableFolder, pdfTableFile = fitsFolder / 'flux_analysis', fitsFile.replace('.fits', '_linesTable_Emission')
    txtTableFolder, txtTableFile = fitsFolder / 'flux_analysis', fitsFile.replace('.fits', '_linesTable_Emission.txt')
    print(f'-{objName}')
