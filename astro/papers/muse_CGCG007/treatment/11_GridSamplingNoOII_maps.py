...
for i, obj in enumerate(objList):
    ...
    grid_fits_file = objFolder/f'{obj}_grid_sampling_NoOII.fits'
    ...
    #               page_hdr=hdr, output_files_prefix='NoOII')

    # ----------------------------------------- Generate the parameter histograms ----------------------------------------

    for param in param_list:

        with fits.open(f'{chemFolder}/NoOII{param}.fits') as hdu_list:
            ...
            for i_mask, mask_items in enumerate(spatial_mask_dict.items()):
                ...
                save_cfg('../muse_CGCG007.ini', store_dict, section_name=f'{mask_name}_GridSampling_NoOII')

            ax.hist(array_container, bins=15, label=data_labels, stacked=True)

            ax.legend()
            ax.update({'title': r'CGCG007−025, {} histogram, Grid sampling, No OII'.format(param_label), 'xlabel': param_label})
            plt.savefig(chemFolder/f'{obj}_{param}_histogram_GridSamplingNoOII')
            # plt.show()
