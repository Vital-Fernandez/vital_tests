...
for i, param in enumerate(param_list):
    ...
    for i_mask, mask_name in enumerate(mask_list):
        row[i_mask + 2] = to_latex_format(obsData[f'{mask_name}_err_direct_method'][f'{param}_array'])

    pdf.addTableRow(row, last_row=False)

pdf.table.add_hline()
pdf.generate_pdf(tablesFolder/'direct_method_ionic_uncertainties')
