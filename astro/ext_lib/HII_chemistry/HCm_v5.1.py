...
output = np.zeros(input1.size, dtype=[('ID', 'U12'), ('OII_3727', float), ('eOII_3727', float), ('NeIII_3868', float),
                                      ('eNeIII_3868', float), ('OIII_4363', float), ('eOIII_4363', float),
                                      ('OIII_5007', float), ('eOIII_5007', float), ('NII_6584', float),
                                      ('eNII_6584', float), ('SII_6725', float), ('eSII_6725', float), ('grid', int),
                                      ('OH', float), ('eOH', float), ('NO', float), ('eNO', float), ('logU', float),
                                      ('elogU', float)])
...
for tab in range(0, input1.size, 1):
    ...
    print(round(100 * (count) / float(len(input1)), 1), '%', Names[tab], grid_type, '', round(OHff, 2), round(eOHff, 2),
...
lineas_header = [' HII-CHI-mistry v.5.1 output file', 'Input file:' + input00, 'Iterations for MonteCarlo: ' + str(n),
...
np.savetxt(input00 + '_hcm-output.dat', output, fmt=' '.join(['%s'] * 1 + ['%.3f'] * 12 + ['%i'] + ['%.2f'] * 6),
           header=header)
