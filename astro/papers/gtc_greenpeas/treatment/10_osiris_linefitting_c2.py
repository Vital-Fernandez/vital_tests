...
for i, obj in enumerate(objList):
        ...
        if i > 1:
            ...
            for ext in ['_BR']:
                ...
                # pdfTableFile = objFolder/f'{obj}{ext}_linesTable_{cycle}'
                # txtTableFile = objFolder/f'{obj}{ext}_linesTable_{cycle}.txt'
                ...
                wave_neb, flux_neb = np.loadtxt(stellarFluxFile, unpack=True)
                wave_stellar, flux_stellar = np.loadtxt(stellarFluxFile, unpack=True)

                # Load line measurer object
                idcs_cropping = (wave >= wmin) & (wave <= wmax)
                wave, flux = wave[idcs_cropping], flux[idcs_cropping]

                lm = sr.LineMesurer(wave, flux - flux_stellar, redshift=z, normFlux=flux_norm)
