...
Rodrigo Nemmen, http://goo.gl/8S1Oo
    ...
    a, b, avar, bvar, covarxiz, covar_ba = numpy.zeros(4), numpy.zeros(4), numpy.zeros(4), numpy.zeros(4), numpy.zeros(
        4), numpy.zeros(4)
    ...
    xi.append(
        xi[0] * (1. + a[1] ** 2) * a[2] / ((a[0] + a[1]) * numpy.sqrt((1. + a[0] ** 2) * (1. + a[1] ** 2))) + xi[1] * (
                    1. + a[0] ** 2) * a[2] / (
                    (a[0] + a[1]) * numpy.sqrt((1. + a[0] ** 2) * (1. + a[1] ** 2))))  # bisector
    xi.append(xi[0] * a[3] / (a[0] ** 2 * numpy.sqrt(4. + (a[1] - 1. / a[0]) ** 2)) + xi[1] * a[3] / numpy.sqrt(
        4. + (a[1] - 1. / a[0]) ** 2))  # orthogonal
    for i in range(4):
...
def bces_regression(x_array, y_array, x_error, y_error, cov=None):
    ...
    if cov == None:
        ...
        cov = numpy.zeros(len(x_array))

    fit_dict['methodology'] = (r'OLS(Y|X)$_{bces}$', r'OLS(X|Y)$_{bces}$', r'bisector$_{bces}$', r'Orthogonal$_{bces}$')

    fit_dict['m'], fit_dict['n'], fit_dict['m_error'], fit_dict['n_error'], fit_dict['cov'] = bces(x_array, x_error,
                                                                                                   y_array, y_error,
                                                                                                   cov)
