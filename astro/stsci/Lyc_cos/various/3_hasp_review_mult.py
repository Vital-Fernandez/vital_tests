...
for i, obj in enumerate(target_list):
    ...
    if i >= 0:
        ...
        # Move the files
        idcs_x1d = (sample_df.index.get_level_values('state') == 'x1d') & (sample_df.object == obj) & (
            sample_df.grating.isin(grating_list))
        ...
        idcs_in = idcs_target & (sample_df.index.get_level_values('state') == 'x1d') & (
            sample_df.grating.isin(grating_list))
        ...
        #     fig, ax = plt.subplots()
        #
        #     for idx_file in sample_df.loc[idcs_in].index:
        #         pname = obs_folder/sample_df.loc[idx_file].filepath
        #         print(idx_file, pname.name, sample_df.loc[idx_file].filecode)
        #         spec = lime.Spectrum.from_file(obs_folder/sample_df.loc[idx_file].filepath, instrument='cos',
        #                                        redshift=cfg_sample['Galaxy_redshifts'][obj])
        #         ax.step(spec.wave_rest, spec.flux, label=pname.stem, linestyle=':', where="mid")
        #
        #     for idx_file in sample_df.loc[idcs_out].index:
        #         pname = obs_folder/sample_df.loc[idx_file].filepath
        #         spec = lime.Spectrum.from_file(pname, instrument='cos', redshift=cfg_sample['Galaxy_redshifts'][obj])
        #         ax.step(spec.wave_rest, spec.flux, label=pname.stem, linestyle='-', color='red', where="mid")
        #
        #     ax.legend()
        #     plt.tight_layout()
        #     plt.show()
        ...
        with plt.rc_context():
            ...
            spec = lime.Spectrum.from_file(pname, instrument='cos', redshift=cfg_sample['Galaxy_redshifts'][obj])
            ax.step(spec.wave_rest, spec.flux, label=pname.stem, linestyle='-', color='red', where="mid")
            lineid_plot.plot_line_ids(spec.wave_rest, spec.flux, line_wave=trans_arr, line_label1=line_labels, ax=ax)
