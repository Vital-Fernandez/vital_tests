...
import numpy as np
from astropy.io import fits
data = np.zeros((400, 400), dtype=np.float64)
hdu = fits.PrimaryHDU(data=data)
header = hdu.header
header['NAXIS1'] = 400
header['NAXIS2'] = 400
header['A'] = 'a'
header['line'] = 'H1_6563A'
header['line2'] = '[O3]_5007A'
# header['O3_5007A_B'] = 'O3_5007A+O3_5007A_K-1'
header['O3_5A'] = 'O3_5007A+O3_5007A_K-1'


address = 'example_file.fits'
hdu.writeto(address, output_verify='warn', overwrite=True)


hdr = fits.getheader(address, ext=0)
for item, value in hdr.items():
    print(item, value)