...
for i, obj in enumerate(objList):
    ...
    for ext in ('_BR', '_B'):
        ...
        lineLog_file = dataFolder/'flux_analysis'/f'{obj}{ext}_linesLog_Emission.txt'
        pdfTableFile = dataFolder/'flux_analysis'/f'{obj}{ext}_linesTable_Emission'
        txtTableFile = dataFolder/'flux_analysis'/f'{obj}{ext}_linesTable_Emission.txt'
        print(f'\n- {i}: {lineLog_file}')
        ...
        flux_spec = flux_array[idx_band][0] if ext in ('_B', '_R') else flux_array

        # Load line measurer object
        lm = sr.LineMesurer(wave_rest[idx_wave], flux_spec[idx_wave], linesDF_address=lineLog_file, normFlux=flux_norm)
        pdf = PdfPrinter()
