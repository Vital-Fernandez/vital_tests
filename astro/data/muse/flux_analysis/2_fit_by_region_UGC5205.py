...
for i, obj in enumerate(objList):
    ...
    if i == 1:
        ...
        fluxImage_Halpha = lineFlux_dict['H1_6563A']
        fluxLevels_Halpha = levelFlux_dict['H1_6563A']
        int_level = fluxLevels_Halpha[-3]#[-4]
        idcs_voxels = np.argwhere(fluxImage_Halpha > int_level)

        # Loop through voxels
        for idx_voxel, idx_pair in enumerate(idcs_voxels):

            print(f'-- Treating voxel {idx_voxel} {idx_pair} of {idcs_voxels.shape[0]}')
